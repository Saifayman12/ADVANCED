...
// Get_deletions() function returns a set of strings that are generated by deleting one character from the input word.
...
//getReplacements() returns a set of strings that are generated by replacing each character of the input word with another character from the alphabet
		    ...
		    private Set<String> Get_Inserted(String wd) {
		        	...
		        	for (int i = 0; i <= wd.length(); i++) {
		        		for (char chr = 'a'; chr <= 'z'; chr++) {
		                	...
		                	insertions.add(string_b.toString());}}
		        			return insertions;}
